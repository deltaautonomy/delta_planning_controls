...
VehicleControl::VehicleControl(double ctrl_freq, double planning_time)
{
    m_ctrl_freq = ctrl_freq;
    planning_time = m_planning_time;
}
// Define member functions
double VehicleControl::getCtrlFreq() { return m_ctrl_freq;} // Getter for control freq
double VehicleControl::getPlanningTime() { return m_planning_time;} // Getter for planning time

void VehicleControl::setCtrlFreq(double new_ctrl_freq) {m_ctrl_freq = new_ctrl_freq;} // Setter for control freq
void VehicleControl::setPlanningTime(double new_planning_time) {m_planning_time = new_planning_time;} //Setter for planning time

// Member function for generating evasive trajectory
MatrixXd VehicleControl::getPolynomialCoefficients(vector<double> boundary_vals)
{
...
MatrixXd VehicleControl::getEvasiveTrajectory(vector<double> boundary_vals)
{
    MatrixXd coeffs = getPolynomialCoefficients(boundary_vals); 
    double T = getPlanningTime();
    double freq = getCtrlFreq();
    double dt = 1/freq;
    int num_steps = (int)T*freq;
